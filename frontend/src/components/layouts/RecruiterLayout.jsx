...
import {
  LayoutDashboard, PlusSquare, Users, ClipboardList,
  MessageSquare, BarChart3, Sun, Moon, LogOut, Bell,
  Menu, X, ChevronRight, Trash2
} from 'lucide-react'
...
const navItems = [
  ...
  { path: '/recruiter/post-job',     label: 'Post a Job',   icon: PlusSquare },
  { path: '/recruiter/candidates',   label: 'Candidates',   icon: Users },
  { path: '/recruiter/applications', label: 'Applications', icon: ClipboardList },
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        {navItems.map(({ path, label, icon: Icon }) => {
          const active = isActive({ path, exact: path === '/recruiter' })
          return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6"
            ...
            <p className="font-body text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>
              This will permanently delete your account, all job postings, applications, and data. This action <strong>cannot be undone</strong>.
            </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body text-xs hidden sm:block" style={{ color: 'var(--text-muted)' }}>
                Recruiter Portal — {user?.company || user?.name}
              </p>
            </div>
          </div>
