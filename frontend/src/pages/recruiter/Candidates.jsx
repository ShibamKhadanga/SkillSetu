...
import { Search, Filter, Eye, MessageSquare, Star, MapPin, GraduationCap, Code, X, Mail, Phone, Users } from 'lucide-react'
    ...
    <div className="w-full max-w-2xl rounded-2xl max-h-[90vh] overflow-y-auto" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
      <div className="p-6">
        ...
        <div className="space-y-4 mb-6">
          ...
          {candidate.email && (
            <div>
              <div className="flex items-center gap-2 mb-2"><Mail size={15} style={{ color: 'var(--accent)' }} /><span className="font-display font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>Contact</span></div>
              <p className="font-body text-sm" style={{ color: 'var(--text-secondary)' }}>{candidate.email} {candidate.phone ? ` · ${candidate.phone}` : ''}</p>
            </div>
          )}
        </div>
...
export default function Candidates() {
    ...
    const fetchCandidates = async () => {
      try {
        ...
        setCandidates([
          ...
          { id: 2, name: 'Rahul Verma', role: 'Full Stack Developer', location: 'Remote', match: 88, skills: ['React', 'Django', 'PostgreSQL', 'AWS'], education: 'B.Tech IT, NIT 2024', avatar: 'RV', email: 'rahul@email.com', phone: '9876543211', bio: 'Full stack developer with strong problem-solving skills.' },
  ...
  const handleContact = (type, candidate) => {
    if (type === 'message') {
      ...
      toast.success(`Opening chat with ${candidate.name}`)
    } else {
      window.open(`mailto:${candidate.email}?subject=Regarding your profile on SkillSetu`)
    }
  }
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            className="input-field pl-9" placeholder="Search by name, skill, role..." />
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 text-center">
          ...
          <p className="font-body text-sm" style={{ color: 'var(--text-muted)' }}>
            Post a job first and candidates will be matched automatically by AI
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
